...
def test_define_dense_model_single_layer():
    ...
    assert model.layers[0].input.shape == (None, 43), " input.shape is not correct"
...
def test_define_dense_model_with_hidden_layer():
    ...
    assert model.layers[0].input.shape == (None, 43), " input.shape is not correct"
    assert model.layers[0].output.shape == (None, 11), " output.shape is not correct"
    assert model.layers[1].output.shape == (None, 1), " output.shape is not correct"
